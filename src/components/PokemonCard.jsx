...
function PokemonCard({ pokemon, onNavigate }) {
  ...
  return (
    <div className="relative p-4 bg-[#FFFCFB] rounded-xl shadow-md hover:shadow-lg shadow-[#FFEAD8] transition-shadow duration-300 flex flex-col gap-3"           onClick={() => handleNavigate(`/pokemon/${pokemon?.name}`)}
>
      <div className="flex justify-between items-center">
        <div className="flex gap-2 bg-white rounded-tl-xl rounded-tr-xl">
          <button
            className={`p-2 w-10 h-10 text-xl flex items-center justify-center rounded-full border-2 transition-all duration-200 z-10
               ${
                 liked
